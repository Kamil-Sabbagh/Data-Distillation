...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None, D_images=None, D_labes=None, percentage = 5000):
    ...
    if dataset == 'CIFAR10':
        ...
        if args and args.zca:
            transform = transforms.Compose([transforms.ToTensor()])
        else:
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        
        # Check if data_file and label files are provided
        if D_images is not None and D_labes is not None:
            # Load images and labels from .pt files
            images = torch.load(D_images)
            labels = torch.load(D_labes)

            # Create a custom dataset using the loaded images and labels
            #print(dst_train)
